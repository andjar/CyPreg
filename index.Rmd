...
ggplotly(
  ggplot(merge(df_aggregated,cytokines),
         aes(ga, ifelse(grepl("log",unit), exp(value), value), shape = condition, color = paste(author, year))) + geom_point() + 
    theme_bw() + scale_y_log10() + theme(legend.position = "bottom") +
    labs(x = "Gestational age", y = "Concentration") + 
    facet_wrap(~printname, scales = "free_y")
)
```
