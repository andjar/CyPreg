...
df_meta <- subset(df_aggregated, cytokine == "IL_6")
df_meta_prep <- data.frame()

df_meta_prep <- rbind(
  df_meta_prep,
  prep_meta(
    dff = subset(df_meta, author == "Ma"),
    Author = "Ma et al. (2019)",
    c = "healthy",
    e = "preeclampsia"
  )
)

dff <- subset(df_meta, author == "Aggarwal")
for(i in unique(dff$ga.string)){
  df_meta_prep <- rbind(
    df_meta_prep,
    prep_meta(
      dff = subset(dff, ga.string == i),
      Author = "Aggarwal et al. (2019)",
      c = "healthy",
      e = "preeclampsia"
    )
  )
...
df_meta_prep <- rbind(
  df_meta_prep,
  prep_meta(
    dff = subset(df_meta, author == "Tangerås"),
    Author = "Tangerås et al. (2015)",
    c = "healthy",
    e = "preeclampsia"
  )
)
...
df_meta_prep <- rbind(
  df_meta_prep,
  prep_meta(
    dff = subset(df_meta, author == "Garcia"),
    Author = "Garcia et al. (2018)",
    c = "healthy",
    e = "preeclampsia"
  )
)

dff <- subset(df_meta, author == "Taylor")
for(i in unique(dff$ga.string)){
  df_meta_prep <- rbind(
    df_meta_prep,
    prep_meta(
      dff = subset(dff, ga.string == i),
      Author = "Taylor et al. (2016)",
      c = "healthy",
      e = "preeclampsia"
    )
  )
...
df_meta_prep <- rbind(
  df_meta_prep,
  prep_meta(
    dff = subset(df_meta, author == "Szarka"),
    Author = "Szarka et al. (2010)",
    c = "healthy",
    e = "preeclampsia"
  )
)
...
```{r, out.height="100%", out.width="100%", fig.width=16, fig.height=8}
df_meta <- subset(df_aggregated, cytokine == "IL_10")
df_meta_prep <- data.frame()
